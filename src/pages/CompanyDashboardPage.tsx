  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        months.push({ month: label, eventos });
      }
    ...
    <div className="container py-8">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        {!checkingApproval && (approved ? (
          <section className="mb-8" aria-labelledby="quick-actions-title">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {(stats?.products || 0) < limits.products ? <Button asChild variant="outline" className="h-auto justify-start p-4"><Link to="/marketplace?cadastrar=produto"><ShoppingBag className="mr-3 h-5 w-5 text-primary" /><span className="text-left"><strong className="block">Cadastrar produto</strong><small className="font-normal text-muted-foreground">{limits.products - (stats?.products || 0)} vagas restantes</small></span></Link></Button> : <Button disabled variant="outline" className="h-auto justify-start p-4"><ShoppingBag className="mr-3 h-5 w-5 text-primary" /><span className="text-left"><strong className="block">Cadastrar produto</strong><small className="font-normal text-muted-foreground">Limite de 5 anúncios atingido</small></span></Button>}
              {(stats?.products || 0) < limits.products ? <Button asChild variant="outline" className="h-auto justify-start p-4"><Link to="/marketplace?cadastrar=servico"><Wrench className="mr-3 h-5 w-5 text-primary" /><span className="text-left"><strong className="block">Cadastrar serviço</strong><small className="font-normal text-muted-foreground">Produtos e serviços: {stats?.products || 0}/5</small></span></Link></Button> : <Button disabled variant="outline" className="h-auto justify-start p-4"><Wrench className="mr-3 h-5 w-5 text-primary" /><span className="text-left"><strong className="block">Cadastrar serviço</strong><small className="font-normal text-muted-foreground">Limite de 5 anúncios atingido</small></span></Button>}
        ...
        <Card className="mt-8">
          ...
          <CardContent><ChartContainer config={chartConfig} className="h-[300px] w-full"><BarChart data={monthlyData}><CartesianGrid strokeDasharray="3 3" className="stroke-border" /><XAxis dataKey="month" className="text-xs" /><YAxis allowDecimals={false} className="text-xs" /><ChartTooltip content={<ChartTooltipContent />} /><Bar dataKey="eventos" fill="var(--color-eventos)" radius={[4, 4, 0, 0]} /></BarChart></ChartContainer></CardContent>
        </Card>
      </motion.div>
    </div>
  );
